...
export default function StatsScreen() {
	...
	const filtered = useMemo(() => {
		...
		if (selectedTagIds.length > 0) {
			result = result.filter((s) =>
				selectedTagIds.some((tagId) => s.tagIds.includes(tagId)),
			);
		}
		return result;
	...
	const zoneStatsForGroup = (groupId: string): ZoneStats[] => {
		const groupZoneIds = basketball.zones
			...
			<ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: 90 }]}>
				...
				{tags.length > 0 && (
					<View style={styles.tagFilterRow}>
						...
						>
							<Text style={[styles.filterText, selectedTagIds.length === 0 && styles.filterTextActive]}>
								All
							</Text>
						</Pressable>
						{tags.map((tag) => (
				...
				{filtered.length === 0 ? (
					...
					<>
						...
						{groupStats.map((gs) => {
							const pct =
								gs.attempted > 0
									? Math.round((gs.made / gs.attempted) * 100)
									: null;
							...
							return (
								<View key={gs.groupId}>
									<Pressable
										...
										onPress={() =>
											setExpandedGroup(isExpanded ? null : gs.groupId)
										}
									>
									...
									{isExpanded && zones.length > 0 && (
										<View style={styles.zoneDetail}>
											{zones
												...
												.map((zs) => {
													const zone = basketball.zones.find(
														(z) => z.id === zs.zoneId,
													);
													const zPct =
														zs.attempted > 0
															? Math.round((zs.made / zs.attempted) * 100)
															: null;
													return (
