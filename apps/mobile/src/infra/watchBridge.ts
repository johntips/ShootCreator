...
export async function sendReliable(message: SyncMessage): Promise<void> {
	...
	try {
		...
		log("Send", "reliable →", message.type, "shotId" in message ? (message as unknown as Record<string, unknown>).shotId : "");
	} catch (e) {
...
export async function sendRealtime(message: SyncMessage): Promise<void> {
	...
	try {
		...
		if (reachable) {
			...
			log("Send", "realtime →", message.type, "shotId" in message ? (message as unknown as Record<string, unknown>).shotId : "");
		} else {
...
/** reachability 変更リスナー */
export async function onReachabilityChange(handler: (reachable: boolean) => void): Promise<UnsubscribeFn> {
	const mod = await getModule();
	if (!mod) return () => {};
...
export async function onMessage(handler: MessageHandler): Promise<UnsubscribeFn> {
	const mod = await getModule();
	if (!mod) return () => {};
	const unsub = mod.watchEvents.on("message", (payload: Record<string, unknown>, reply: unknown) => {
...
export async function onUserInfo(handler: MessageHandler): Promise<UnsubscribeFn> {
	...
	const unsub = mod.watchEvents.on("user-info", (payloads: Record<string, unknown>[]) => {
		log("Recv", `user-info ← ${payloads.length} item(s)`, payloads.map((p) => p.type));
